...
#     $env:CLAUDE_GLM_TEST_ERROR=1; iwr -useb https://raw.githubusercontent.com/Blurjp/claude-glm-wrapper/main/install.ps1 | iex; $env:CLAUDE_GLM_TEST_ERROR=$null
#
#   Enable debug mode:
#     $env:CLAUDE_GLM_DEBUG=1; iwr -useb https://raw.githubusercontent.com/Blurjp/claude-glm-wrapper/main/install.ps1 | iex; $env:CLAUDE_GLM_DEBUG=$null
#
...
function New-ClaudeGlm5Wrapper {
    $wrapperPath = Join-Path $UserBinDir "claude-glm-5.ps1"
    ...
    $wrapperContent = @(
        ...
        '',
        '# Launch Claude Code with custom config',
        'Write-Host "LAUNCH: Starting Claude Code with GLM-5..."',
        'Write-Host "CONFIG: Config directory: $env:CLAUDE_HOME"',
        'Write-Host ""',
        '',
        '# Check if claude exists',
    ...
    Set-Content -Path $wrapperPath -Value $wrapperContent
    Write-Host "OK: Installed claude-glm-5 at $wrapperPath" -ForegroundColor Green
}
...
function Report-Error {
    ...
    if ($reportChoice -ne "y" -and $reportChoice -ne "Y") {
        ...
        Write-Host "  https://github.com/Blurjp/claude-glm-wrapper/issues" -ForegroundColor Cyan
        Write-Host ""
    ...
    $issueUrl = "https://github.com/Blurjp/claude-glm-wrapper/issues/new?title=$encodedTitle`&body=$encodedBody`&labels=bug,windows,installation"
    ...
    if (-not $browserOpened) {
        ...
        $shortUrl = "https://github.com/Blurjp/claude-glm-wrapper/issues/new?title=$encodedTitle`&labels=bug,windows,installation"
...
function Install-ClaudeGlm {
    ...
    if ($inputKey) {
        ...
        Write-Host "   $UserBinDir\claude-glm-5.ps1"
    ...
    Write-Host "   claude-glm-5     - GLM-5 (explicit)"
    ...
    Write-Host "   ccg5  - claude-glm-5 (GLM-5, explicit)"
    ...
    Write-Host "LOCATION: Config directories: $GlmConfigDir, $Glm5ConfigDir, $Glm46ConfigDir, $Glm45ConfigDir, $GlmFastConfigDir"
}
...
if ($TestError) {
    ...
    Write-Host "   iwr -useb https://raw.githubusercontent.com/Blurjp/claude-glm-wrapper/main/install.ps1 | iex" -ForegroundColor Cyan
    Write-Host ""
